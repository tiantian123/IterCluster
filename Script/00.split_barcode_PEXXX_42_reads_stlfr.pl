...
my (@Read21, @Read22, @Read23,@Read24);
...
while(<IN2>){
  ...
  if($n % 4 == 2){
    ...
    if((exists $barcode_hash{$b1}) && (exists $barcode_hash{$b2}) && (exists $barcode_hash{$b3})){
      ...
      $Read21[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $id."\#$hash\/2\t$index_hash{$hash}\t1";
      $Read22[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $read;
      $T = <IN2>; $n++;chomp($T);
      $Read23[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $T;
      $T = <IN2>; $n++;chomp($T);
      my $qual = substr($T,0,$read_len);
      $Read24[$index_hash{$hash}][$Read_num[$index_hash{$hash}]] = $qual;
      $T = <IN1>; chomp($T);
      ...
      $Read_num[0] ++;
      $Read21[0][$Read_num[0]] = $id."\#0_0_0\/2\t0\t1";
      $Read22[0][$Read_num[0]] = $read;
      $T = <IN2>; $n++;chomp($T);
      $Read23[0][$Read_num[0]] = $T;
      $T = <IN2>; $n++;chomp($T);
      my $qual = substr($T,0,$read_len);
      $Read24[0][$Read_num[0]] = $qual;
      $T = <IN1>; chomp($T);
...
open OUT2, ">split_reads.log" or die "Can't write file";
...
for(my $i=1;$i<=$split_barcode_num;$i++){
  ...
  for(my $j=1;$j<=$Read_num[$i];$j++){
    print OUT "$Read1[$i][$j]\n$Read2[$i][$j]\n$Read3[$i][$j]\n$Read4[$i][$j]\n";
    print OUT3 "$Read21[$i][$j]\n$Read22[$i][$j]\n$Read23[$i][$j]\n$Read24[$i][$j]\n";
  }
}

for(my $j=1;$j<=$Read_num[0];$j++){
    print OUT "$Read1[0][$j]\n$Read2[0][$j]\n$Read3[0][$j]\n$Read4[0][$j]\n";
    print OUT3 "$Read21[0][$j]\n$Read22[0][$j]\n$Read23[0][$j]\n$Read24[0][$j]\n";
  }
