...
bool SeqReader::get(SeqRecord& sr)
{
    ...
    if(validRecord)
    {
        ...
        // Get barcode
        if( endPos != std::string::npos && header.size() - endPos >=2 )
        {
            sr.barcode = atoi(header.substr(endPos).c_str());
        }
        else
        {
            std::cerr<<"No barcode found from "<<header<<std::endl;
            abort();
        }
        ...
        if( !(m_flags & SRF_NO_VALIDATION) )
        {
            ...
            {
                std::cerr << "Error: read " << header << " contains non-ACGT characters.\n";
