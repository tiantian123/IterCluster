...
use strict;
my $usage =<<USAGE;
version: v5, 2018-06-21
example: perl $0  <seed_capture_result file> <bar_pos file> <seed-info file>  <LFR low len> <reads_low_cnt> <expand_len> <real_bar_thr>
USAGE
die $usage if(@ARGV != 7);
...
open (OUT,">evaluate.txt") or die("can't create the file!\n");
...
while(<IN1>){
  ...
  my @chr = split(/\./, $line[1]);
  next if ($line[4] <= $LFR_len_thr && $line[5] <= $reads_low_cnt);
...
print OUT "seed_id LFR_location unique_kme kmer_cnt   rate   correct_cnt  real_cnt   cap_cnt   ppv  fdr  fnr\n";
...
while(<IN>){
  ...
  if (@seed1 >= @seed2 ){         #search seed barcode info
     Match_seed(@line,$line[0]);
  }elsif(@seed2 > @seed1){
     Match_seed(@line,$line[1]);
  }
# print OUT "\n";
 $seed_cnt ++;   
}
...
#  print "$bar_strand\n";
  ...
  while (my ($key,$value) = each %$bar_strand){
         my @bar = split(/\t/,$value);
         if ($bar[4] > $LFR_len_thr && $bar[5] > $reads_low_cnt){ #add the condition: the initial seed's count should be larger than reads_low_cnt
           @True_well_sub = evaluate($bar[0],$bar[1],$bar[5],$expand_len);
     ...
     my %hash_True = map{$_=>1} @True_well;
     my %hash_cap = map{$_=>1} @Cluster_set;
     my @common = grep {$hash_True{$_}} @Cluster_set;

     my $real_cnt = @True_well;
     my $cap_cnt = @Cluster_set;
